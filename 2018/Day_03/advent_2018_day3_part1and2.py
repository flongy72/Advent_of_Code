...
pos=0
for co_ords in location:                    # iterate through array once complete
    overlap=False                           # and create overlap=True if a value of 2
    steve=co_ords.split(",")                # is found in any claim
    x,y = int(steve[0]),int(steve[1])
    new_area = area[pos]
    bob=new_area.split("x")
    a,b = int(bob[0]), int(bob[1])
    ...
    if not overlap:
        print (id[pos])                         # print the equivalent index of the id list
        break                                   # for the claim which no overlap is found
    pos += 1
